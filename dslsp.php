...
if($act == 'del' && isset($_SESSION['admin'])) {
	...
	if($_POST['adddone'] != '') {
		...
		$sql = $mysql->query("INSERT INTO $table VALUES ('$dataColID','$dataColID2')");
	...
	if($_POST['editdone'] != '') {
		...
		//$dataColID3 = addslashes($_POST[$ColID3]);
		//$dataColID4 = addslashes($_POST[$ColID4]);
		$sql = $mysql->query("UPDATE $table SET $ColID='$dataColID', $ColID2='$dataColID2' WHERE $ColID='$IDact'");
...
echo '<thead><tr><td colspan = "5" style="text-align:center;"><b> Danh sách loại sản phẩm </b></td></tr></thead>';
echo '<thead>';
echo '<tr><th>MaLoaiSP <a href="?sort1='.(($_GET['sort1'] == 'ASC') ? 'DESC">DESC' : 'ASC">ASC').'</a></th>'; // ID
echo '<th>MoTa <a href="?sort2='.(($_GET['sort2'] == 'DESC') ? 'ASC">ASC' : 'DESC">DESC').'</a></th>'; // Name
echo '<th>Act</th>';
echo '</tr></thead><tbody>';
...
<a href="?act=add">Thêm loại sản phẩm</a>
<br /><br /><?php
