...
<meta name="title" content="Danh sách khách hàng">
...
echo '<thead><tr><td colspan = "5" style="text-align:center;"><b> Danh sách khách hàng </b></td></tr></thead>';
echo '<thead>';
echo '<tr><th>MaKH <a href="?sort1='.(($_GET['sort1'] == 'ASC') ? 'DESC">DESC' : 'ASC">ASC').'</a></th>'; // ID
echo '<th>TenKH <a href="?sort2='.(($_GET['sort2'] == 'DESC') ? 'ASC">ASC' : 'DESC">DESC').'</a></th>'; // Name
echo '<th>DiaChi <a href="?sort3='.(($_GET['sort3'] == 'DESC') ? 'ASC">ASC' : 'DESC">DESC').'</a></th>'; // Name
echo '<th>Email <a href="?sort4='.(($_GET['sort4'] == 'DESC') ? 'ASC">ASC' : 'DESC">DESC').'</a></th>';
echo '<th>Act</th>';
echo '</tr></thead><tbody>';
while($data=mysql_fetch_array($limit)) {
	echo '<tr style="">';
	echo '<td>'.$data[$ColID].'</td>'; // MaKH
	echo '<td>'.$data[$ColID2].'</td>'; // TenKH
	echo '<td>'.$data[$ColID3].'</td>'; // DiaChi
	echo '<td>'.$data[$ColID4].'</td>'; // Email
	echo '<td><a href="?act=edit&IDact='.$data[$ColID].'">Edit</a> - <a href="?act=del&IDact='.$data[$ColID].'">Del</a></td>';
	echo '</tr>';
}
?>
</tbody></table><br />
<a href="?act=add">Thêm khách hàng mới</a>
<br /><br /><?php
